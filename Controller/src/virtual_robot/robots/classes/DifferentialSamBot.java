 ...
 * <p>
 * DifferentialBot is the controller class for the "differential_bot.fxml" markup file.
 */
@BotConfig(name = "Differential Sam Bot", filename = "differential_bot")
public class DifferentialSamBot extends VirtualBot {
	...
	private final double STEER_RATIO = (18 / 15.0) * (32 / 13.0) * (107 / 32.0);
	private final double DRIVE_RATIO = STEER_RATIO * (18 / 63.0);

	//		String[] motorNames = new String[]{ "bottomLeft", "topLeft", "bottomRight", "topRight" };
	private String[] motorNames = new String[]{ "bottom_left_motor", "top_left_motor", "bottom_right_motor", "top_right_motor" };
	private double maxWheelForce;

	public DifferentialSamBot( ) {
		super( );
	}
	...
	public void initialize( ) {
		...
		motors = new DcMotorExImpl[]{
				hardwareMap.get( DcMotorExImpl.class, motorNames[0] ),
				hardwareMap.get( DcMotorExImpl.class, motorNames[1] ),
				hardwareMap.get( DcMotorExImpl.class, motorNames[2] ),
				hardwareMap.get( DcMotorExImpl.class, motorNames[3] )
		};
	...
	public synchronized void updateStateAndSensors( double millis ) {
		...
		for( int i = 0; i < 2; i++ ) {
			...
			if( i == 0 && (MOTOR_TYPE.REVERSED && !bottomRev || !MOTOR_TYPE.REVERSED && bottomRev) || i == 1 && (MOTOR_TYPE.REVERSED && bottomRev || !MOTOR_TYPE.REVERSED && !bottomRev) )
				bottomSpd = -bottomSpd;

			double topSpd = motors[2 * i + 1].getVelocity( );
			if( i == 0 && (MOTOR_TYPE.REVERSED && !topRev || !MOTOR_TYPE.REVERSED && topRev) || i == 1 && (MOTOR_TYPE.REVERSED && topRev || !MOTOR_TYPE.REVERSED && !topRev) )
				topSpd = -topSpd;
