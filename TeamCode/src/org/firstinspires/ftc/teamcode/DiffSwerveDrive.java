...
public class DiffSwerveDrive {
	...
	public static final double MAX_DRIVE_SPEED = 1.5 * MAX_TICKS_PER_SECOND * WHEEL_CIRCUMFERENCE / TICKS_PER_ROTATION;
	...
	public void init( HardwareMap hardwareMap ) {
		init( hardwareMap, "bottom_left_motor", "top_left_motor", "bottom_right_motor", "top_right_motor" );
	}

	public void init( HardwareMap hardwareMap, String... motorNames ) {
		for( int i = 0; i < 4; i++ ) {
	...
	public void setDriveSpeed( float vx, float vy, float va ) {
		...
		for( int i = 0; i < 4; i++ )
			motors[i].setPower( motorPowers[i] / max );
